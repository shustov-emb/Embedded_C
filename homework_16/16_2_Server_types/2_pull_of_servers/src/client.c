...
void *TalkToServer(void *args)
{
    ...
    // то значит что все дочерние серверы заняты другими соединениями
    // Прекращаеаем работу клиента
    if (!strcmp(endpoint, "BUSY"))
    {
        printf("All child servers are busy\n");
        return NULL;
    }
    ...
    recv_bytes = recv(conn->fd, buffer, sizeof(buffer) - 1, 0);
    if (recv_bytes <= 0)
    {
        perror("recv error");
        close(conn->fd);
