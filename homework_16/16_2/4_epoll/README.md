...
* Сервер в свою очередь создаёт два сервера на разных портах и разных типах сокета, в одном потоке, через epoll начинает слушать события на дескрипторах серверов, и в зависимости от того на какой дескриптор падает событие, тот дескриптор и идёт обрабатывать клиента
