 ...
 * @brief Простая реализация, принимающего raw сокета
 * Для проверки работоспособности можно воспользоваться коммандой - echo "Some data" | nc -u 127.0.0.1 777
 * @version 0.1
 * @date 2026-05-06
 * 
...
int main(void)
{
    ...
    size_t recieved_packets = 0;

    printf("\n");

    while (recieved_packets < 10)
    {
        ...
        //Достаём данные, смешаем начало буфера на длины ip и udp заголовка, udp заголовок насколько я понял - стандарный - 8 байт
        ...
        //Выводим пакет, но мало какие пакеты мы сможем прочитать, мало кто строками информацию отправляет
        //Но для теста нагуглил команду: echo "Some data" | nc -u 127.0.0.1 777 
        //Можно на адресс и порт послать свои udp данные 
        printf("%-10s: %s:%d -> %s:%d\n","Packet",
        ...
        //По новой вспомнил про форматированный красивый вывод, надо бы не забывать пользоваться им
