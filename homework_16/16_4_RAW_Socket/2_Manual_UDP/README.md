#16.4.2 UDP Заголовок

RAW Клиентом отправляем сообщение серверу в ручную поменяв `UDP` заголовок (сервер я взял из `homework_16/16_1_AF_FAMILY/AF_INET_SOC_DGRAM`), и RAW сокетом слушаем нужный порт, получаем изменённое сообщенеие от сервера, выводим на экран.
